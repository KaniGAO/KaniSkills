...
def generate_calendar() -> list[dict]:
    ...
    # 过滤：保留过去一周(含今天)起、至未来 2 个月的事件，
    # 让"重要会议"那张表也能回溯刚发生的一周事件（状态由报告层标 Past/Today/Upcoming）
    start = (hkt - timedelta(days=7)).strftime("%Y-%m-%d")
    cutoff = (hkt + timedelta(days=60)).strftime("%Y-%m-%d")
    filtered = [e for e in events if e["date"][:10] >= start and e["date"][:10] <= cutoff]
