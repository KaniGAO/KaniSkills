...
def run(send_email: bool = False, output_path: str = None) -> str:
    """运行日报生成流程"""
    # 初始化审计/校验存储层
    db.init_db()
    ...
    if not output_path:
        ...
        output_path = os.path.join(output_dir, f"global-markets-briefing-{date_str}.docx")
    ...
    if send_email:
        ...
        print("  Sending Email...")
        print(f"{'=' * 50}")
        success = send_report(report_path)
        if success:
...
if __name__ == "__main__":
    ...
    run(send_email=send_email, output_path=output_path)
